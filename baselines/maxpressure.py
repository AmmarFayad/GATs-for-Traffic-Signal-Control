...
class Simulation:
    ...
    def run(self, episode, args):
        ...
        self.step = 0
        old_total_wait = 0
        action=[random.randint(0,1) for _ in range (args.num_agents)]
        ...
        while self.step < args.num_steps:
            ...
            for i, junc in enumerate(junctions):
                if self.step %2==0: action[i]= np.argmax (self.pressure(junc))
                #print(action)
                # if the chosen phase is different from the last phase, activate the yellow phase
                if self.step != 0 and old_action[i] != action[i]:
                    self.set_yellow_phase(id=junc,old_action=action[i])
                    self.simulate(junc, self.yellow_duration, is_green=False)
                
                
                # execute the phase selected before
                self.set_green_phase(id=junc,action_number=action[i])
        ...
        return simulation_time, training_time, avg_reward, avg_waiting, total_loss, dist_entropy

    def nextlane(self, l):
        nl=l[2]+l[3]
        if ord(l[0])==ord(l[2])+1: nl+=chr(ord(l[2])-1)+str(l[1])
        elif ord(l[0])==ord(l[2])-1: nl+=chr(ord(l[2])+1)+str(l[1])
        elif ord(l[0])==ord(l[2]): nl+=nl[0]+str(2-int(l[1]))
        return nl

    def pressure (self, id):
        a=[0,0]
        for inc in traci.trafficlight.getControlledLanes(id):
            outg=self.nextlane(inc)+'_0'
            c=traci.lane.getLastStepVehicleNumber(inc)
            ll=traci.lane.getIDList()
               
            g=traci.lane.getLastStepVehicleNumber(outg)
            diff=c-g
            if inc[0]==inc[2]: a[0]+=diff
            else: a[1]+=diff        
        return np.array(a)
    def simulate(self, id, steps_todo, is_green):
        """
        ...
        while steps_todo > 0:
            ...
            steps_todo -= 1
            queue_length = self.get_queue_length(id)
