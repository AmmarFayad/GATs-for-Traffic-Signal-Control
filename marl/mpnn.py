...
import numpy as np
from marl.rlcore.distributions import Categorical
...
class MPNN(nn.Module):
    def __init__(self, action_space, num_agents, num_entities, input_size=16, hidden_dim=128, embed_dim=None,
                 pos_index=2, norm_in=False, nonlin=nn.ReLU, n_heads=1, mask_dist=None, entity_mp=False, num_actions=4):
        super().__init__()
        ...
        if self.mask_dist is not None and self.mask_dist > 0: 
            for i in range(1,self.num_agents):
                ...
                for x in range(self.num_agents):
                    if abs(x-i)<=4 or abs(x-i)!=0:
                        mask[:,x,(x+i)%self.num_agents].copy_(torch.tensor([1]))
                    #mask[:,x,(x+i)%self.num_agents].copy_(restrict[bsz*x:bsz*(x+1)])
        
        elif self.mask_dist is not None and self.mask_dist <0:# -10:
