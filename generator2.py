...
class TrafficGenerator:
    ...
    def generate_routefile(self, seed):
        ...
        with open("intersection/3intersection/network.rout.xml", "w") as routes:
            print("""<routes>
                    <vType accel="1.0" decel="4.5" id="standard_car" length="5.0" minGap="2.5" maxSpeed="25" sigma="0.5" />
                    
                    <route id="0"  edges="A1B1 B1C1 C1D1 D1E1" />
                    <route id="1"  edges="E1D1 D1C1 C1B1 B1A1" />
                    <route id="2"  edges="B2B1 B1B0" />
                    <route id="3"  edges="B0B1 B1B2" />
                    <route id="4"  edges="C2C1 C1C0" />
                    <route id="5"  edges="D2D1 D1D0" />
                    <route id="6"  edges="C0C1 C1C2" />
                    <route id="7"  edges="D0D1 D1D2" />
        ...
        #     """, file=routes)
            ...
            for car_counter, step in enumerate(car_gen_steps):
                ...
                if True:
                    ...
                    route_straight = np.random.choice([0,1,2,3,4,5,6,7], size=1, p=[0.3,0.3, 0.1,0.1,0.05,0.05,0.05,0.05])[0]#np.random.randint(0, 2)
