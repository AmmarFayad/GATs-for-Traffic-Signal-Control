...
class Simulation:
    ...
    def run(self, episode, args):
        ...
        #if episode>190: self.sumo_cmd=set_sumo(True, args.sumocfg_file_name, args.max_steps)
        traci.start(self.sumo_cmd)
        if self.step==0: 
            self.old_car_list = traci.vehicle.getIDList()
            for car in self.old_car_list:
                self.cars_travel_time[car]=1
        ...
        while self.step < args.num_steps:
            ...
            with torch.no_grad():
                action = self.master.act(self.step)
        ...
        return travel_time, training_time, avg_reward, avg_waiting, total_loss, dist_entropy
    ...
    def update_travel_time(self):
        ...
        for car in car_list:
            if car not in self.old_car_list: 
                self.cars_travel_time[car]=1
            else: self.cars_travel_time[car]+=1
        self.old_car_list=car_list

    def avg_travel_time(self):
        return np.mean([b for _,b in self.cars_travel_time.items()])
    ...
    def get_state_general(self,args,junctions):
        ...
        for car_id in car_list:
            ...
            if traci.vehicle.getNextTLS(car_id) is not () and traci.vehicle.getNextTLS(car_id)[0][0] in junctions:
                if traci.vehicle.getNextTLS(car_id) is not ():
                    lane_pos=traci.vehicle.getNextTLS(car_id)[0][2]
                else: lane_pos=750
